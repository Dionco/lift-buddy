# Powerlifting Expert Agent Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a development-time powerlifting expert system: a comprehensive knowledge base and a Claude Code skill, with CLAUDE.md wired to load the knowledge base as background context every session.

**Architecture:** Three files: `docs/powerlifting-knowledge.md` (the knowledge base — primary artifact and future system prompt), `.claude/skills/plift.md` (project-scoped skill for explicit consultations), and an update to `CLAUDE.md` to reference the knowledge base.

**Tech Stack:** Markdown only — no code changes.

**Spec:** `docs/superpowers/specs/2026-03-24-powerlifting-agent-design.md`

---

### Task 1: Write the powerlifting knowledge base

**Files:**
- Create: `docs/powerlifting-knowledge.md`

- [ ] **Step 1: Create `docs/powerlifting-knowledge.md` with the following content**
...
```bash
wc -l docs/powerlifting-knowledge.md
head -5 docs/powerlifting-knowledge.md
```

Expected: file exists, line count around 200+, starts with `# Powerlifting Knowledge Base`

- [ ] **Step 3: Commit**

```bash
git add docs/powerlifting-knowledge.md
git commit -m "docs: add evidence-based powerlifting knowledge base

Comprehensive reference covering RPE/RIR, e1RM, progressive overload,
fatigue/recovery, volume landmarks, periodization, rest periods, and
main lift specifics. Dual-purpose: dev-time context and future in-app
...
### Task 2: Create the `/plift` Claude Code skill

**Files:**
- Create: `.claude/skills/plift.md`

- [ ] **Step 1: Create `.claude/skills/plift.md` with the following content**
...
```bash
cat .claude/skills/plift.md | head -5
```

Expected: starts with `# Powerlifting Expert Consultant`

- [ ] **Step 3: Commit**

```bash
git add .claude/skills/plift.md
git commit -m "feat: add /plift powerlifting expert skill

Project-scoped Claude Code skill for domain consultations during
lift-buddy development. Invoked with /plift <question>.
...
### Task 3: Update CLAUDE.md to reference the knowledge base

**Files:**
- Modify: `CLAUDE.md`

- [ ] **Step 1: Add a domain knowledge section to `CLAUDE.md`**
...
- [ ] **Step 2: Verify CLAUDE.md ends with the new section**

```bash
tail -10 CLAUDE.md
```

Expected: shows the Domain Knowledge section

- [ ] **Step 3: Commit**

```bash
git add CLAUDE.md
git commit -m "docs: wire powerlifting knowledge base into CLAUDE.md

Adds domain knowledge section pointing to docs/powerlifting-knowledge.md
so Claude Code loads it as background context every session.
...
- [ ] Confirm all three files are in place:

```bash
ls docs/powerlifting-knowledge.md .claude/skills/plift.md CLAUDE.md
```
