...
import { e1rm, topSet, topSetE1rm, lastTopSet, recentTopSetE1RMs } from '@/lib/e1rm';
...
describe('e1rm', () => {
  it('matches the CONTEXT.md formula for a sub-maximal set', () => {
    // 100 / (1.0278 - 0.0278 × (5 + (10 - 8))) = 100 / (1.0278 - 0.0278 × 7)
    expect(e1rm(100, 5, 8)).toBeCloseTo(100 / (1.0278 - 0.0278 * 7), 6);
  });

  it('returns the weight itself at a true 1RM (effective reps ≤ 0)', () => {
    expect(e1rm(200, 1, 10)).toBe(200);
  });

  it('same weight × reps at lower RPE implies a higher max (more reps in reserve)', () => {
    expect(e1rm(100, 5, 7)).toBeGreaterThan(e1rm(100, 5, 9));
  });
});
...
describe('topSet', () => {
  it('returns null when no completed sets', () => {
    ...
    expect(topSet([set(0, 0, 7, false)])).toBeNull();
  });
  ...
  it('selects the set with the highest e1RM, not the highest weight', () => {
    ...
    const top = topSet([heavyLowEffort, lightHighEffort]);
...
describe('topSetE1rm', () => {
  it('returns 0 when there is no completed set', () => {
    expect(topSetE1rm([])).toBe(0);
    expect(topSetE1rm([set(0, 5, 7), set(100, 0, 7)])).toBe(0);
  });

  it('returns the e1RM of the top set', () => {
    const sets = [set(150, 3, 7), set(100, 5, 10)];
    expect(topSetE1rm(sets)).toBeCloseTo(e1rm(150, 3, 7), 6);
  });
});
...
describe('recentTopSetE1RMs', () => {
  ...
  it('caps at the requested limit, newest first', () => {
    ...
    expect(out[0]).toBeCloseTo(e1rm(120, 3, 8), 4);
  });
});
